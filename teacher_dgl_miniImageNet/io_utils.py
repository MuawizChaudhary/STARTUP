...
def create_resnet12():
    m = models.Resnet12(width=1, dropout=0.1)
    m.final_feat_dim = m.output_size
    return m

def create_resnet18():
    m = models.resnet18(remove_last_relu=False,
                        input_high_res=True)
...
def create_dgl():
    m = models.VGGn('vgg11', feat_mult=1, num_classes=64, dropout=0.0,nonlin='relu',
...
#def create_dglresnet():
#    m = models.ResNet10_dgl
#    m.final_feat_dim = 512
#    return m
...
def parse_args(script):
    ...
    parser.add_argument('--subset_split',        help='split for dataset')
