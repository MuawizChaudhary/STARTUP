...
class Classifier(nn.Module):
    def __init__(self, dim, n_way):
        ...
        self.fc = nn.Sequential(
            ...
            View(dim),
            Linear_Layer_Local_Loss(dim*2, n_way))
    def forward(self, x):
        x, _ = self.fc(x)
        return x
...
def finetune(novel_loader, params, n_shot): 
    ...
    if params.model == 'resnet10':
        ...
        pretrained_model_template =  models.VGGn('vgg11', feat_mult=1, num_classes=64, dropout=0.0,nonlin='relu',
        ...
        pretrained_model_template = 
         ...
         #= models.VGGn('vgg11', feat_mult=1, num_classes=64, dropout=0.0,nonlin='relu',
         #             loss_sup='pred', dim_in_decoder=200704, num_layers=1,
         #   num_hidden=1024, aux_type='nokland', n_mlp=1, n_conv=0,
         #   pooling='adaptiveavg', bn=True, aux_bn=False)
        feature_dim = 512
...
#    sd.pop("main_cnn.blocks.9.linear.bias")
    ...
    for i, (x, y) in tqdm(enumerate(novel_loader)):
        ...
        n = len(pretrained_model.main_cnn.blocks ) - 2
        ...
        for epoch in range(total_epoch):
            ...
            for j in range(0, support_size, batch_size):
                ...
                if params.freeze_backbone:
                    ...
                    _, _, output = pretrained_model(z_batch, n, True)
        ...
        with torch.no_grad():
            _,_,output = pretrained_model(x_b_i, n, True)
